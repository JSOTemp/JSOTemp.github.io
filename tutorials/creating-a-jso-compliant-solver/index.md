...
```
Error: UndefVarError: nlp not defined
```
...
```
Error: UndefVarError: obj not defined
```
...
```
Error: ArgumentError: Package SolverCore not found in current path:
...
```
Error: UndefVarError: newton not defined
```
...
```
Error: UndefVarError: AbstractNLPModel not defined
```
...
```
Error: UndefVarError: SolverCore not defined
```
...
```
Error: UndefVarError: ADNLPModel not defined
```
...
```
Error: ArgumentError: Package SolverBenchmark not found in current path:
- Run `import Pkg; Pkg.add("SolverBenchmark")` to install the SolverBenchma
rk package.
```
...
```
Error: UndefVarError: performance_profile not defined
```
